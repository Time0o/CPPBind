...
{
  {
    struct test_pointer pointer;
    test_pointer_new(1, &pointer);

    struct test_pointee pointee1;
    test_pointer_deref(&pointer, &pointee1);
    assert(test_pointee_get_state(&pointee1) == 1);

    test_pointee_set_state(&pointee1, 2);
    struct test_pointee pointee2;
    test_pointer_deref(&pointer, &pointee2);
...

  {
    ...
    test_class_with_callable_member_new(&class);
